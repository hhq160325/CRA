...
const CarDetail = () => {
    ...
    const { favoriteCarIds } = useSelector((state) => state.favorites);
    ...
    const handleToggleFavorite = async (carId) => {
        ...
        // Optimistic update
        dispatch(toggleFavoriteLocal(carId));
        
        try {
            if (isFavorite) {
                await dispatch(removeFromFavorites(carId)).unwrap();
            } else {
                await dispatch(addToFavorites(carId)).unwrap();
            }
        } catch (error) {
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border">
            <div className="flex justify-between items-start mb-4">
                ...
                >
                    <svg 
                        className="w-6 h-6" 
                        fill={favoriteCarIds.includes(car.id) ? "currentColor" : "none"} 
                        stroke="currentColor" 
                        viewBox="0 0 24 24"
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
                    ...
                    <div className="bg-white rounded-lg p-8">
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <button 
                                onClick={() => handleToggleFavorite(carData.id)}
                                className="text-red-500 hover:text-red-600 p-2 transition-colors"
                            >
                                <svg 
                                    className="w-7 h-7" 
                                    fill={favoriteCarIds.includes(carData.id) ? "currentColor" : "none"} 
                                    stroke="currentColor" 
                                    viewBox="0 0 24 24"
                                >
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
