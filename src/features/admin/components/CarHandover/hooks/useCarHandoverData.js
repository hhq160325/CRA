...
export const useCarHandoverData = () => {
  const [carHandoverLogs, setCarHandoverLogs] = useState([]);
  const [staffMap, setStaffMap] = useState({});
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchCarHandoverLogs = async () => {
      try {
        setLoading(true);

        // Fetch car handover logs and all users in parallel
        const [logsResponse, usersResponse] = await Promise.all([
          axiosInstance.get(AUDIT_LOG_ENDPOINTS.GET_CAR_HANDOVER_LOGS),
        ...
        setCarHandoverLogs(sortedLogs);
        setError(null);
      } catch (err) {
        console.error('Error fetching car handover logs:', err);
        setError('Failed to load car handover logs');
      } finally {
        setLoading(false);
      }
    };

    fetchCarHandoverLogs();
  }, []);
