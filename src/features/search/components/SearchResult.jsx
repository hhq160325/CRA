...
const SearchResult = () => {
  ...
  // Helper function to parse seat range and check if car matches
  const matchesSeatRange = (car, seatRange) => {
    if (!seatRange) return true;
    
    let minSeats, maxSeats;
    
    // Remove spaces for processing
    const cleanRange = seatRange.replace(/\s/g, '');
    
    // Check if it's in explicit range format (e.g., "3-4", "5-7")
    const rangeMatch = cleanRange.match(/^(\d+)-(\d+)$/);
    if (rangeMatch) {
      minSeats = parseInt(rangeMatch[1]);
      maxSeats = parseInt(rangeMatch[2]);
    } 
    // Check if it's a two-digit number that should be interpreted as range (e.g., "34" -> "3-4")
    else if (/^\d{2}$/.test(cleanRange)) {
      const digits = cleanRange.split('');
      minSeats = parseInt(digits[0]);
      maxSeats = parseInt(digits[1]);
    }
    // Single digit or other format - try exact match
    else {
      return car.seats?.toString() === cleanRange ||
             (typeof car.capacity === 'string' && car.capacity.includes(cleanRange));
    }
    
    // Get car seat count
    let carSeatCount;
    if (typeof car.seats === 'number') {
      carSeatCount = car.seats;
    } else if (typeof car.seats === 'string') {
      carSeatCount = parseInt(car.seats);
    } else if (typeof car.capacity === 'string') {
      // Extract number from capacity string like "4 People"
      const capacityMatch = car.capacity.match(/(\d+)/);
      carSeatCount = capacityMatch ? parseInt(capacityMatch[1]) : null;
    }
    
    if (carSeatCount === null || isNaN(carSeatCount)) return false;
    
    return carSeatCount >= minSeats && carSeatCount <= maxSeats;
  };
  ...
  const filteredCars = useMemo(() => {
    return displayCars.filter(car => {
      ...
      const manufacturerFilter = urlParams.manufacturer.toLowerCase();
      const modelFilter = urlParams.model.toLowerCase();
      ...
      const matchesUrlSeats = matchesSeatRange(car, seatsFilter);

      const matchesManufacturer = !manufacturerFilter || 
                                 car.manufacturer?.toLowerCase().includes(manufacturerFilter) ||
                                 car.brand?.toLowerCase().includes(manufacturerFilter);

      const matchesModel = !modelFilter || 
                          car.model?.toLowerCase().includes(modelFilter) ||
                          car.name?.toLowerCase().includes(modelFilter);

      // If URL parameters don't match, exclude the car
      if (!matchesSearch || !matchesUrlFuel || !matchesUrlSeats || !matchesManufacturer || !matchesModel) {
        return false;
      }
      ...
      <div className="max-w-7xl mx-auto py-4 px-4 sm:py-8 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-8">
          ...
          <div className="flex-1">
            ...
            {(urlParams.q || urlParams.fuel || urlParams.seats || urlParams.manufacturer || urlParams.model) && (
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {urlParams.manufacturer && (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-orange-100 text-orange-800">
                      {t('manufacturer') || 'Manufacturer'}: {urlParams.manufacturer}
                      <button
                        onClick={() => {
                          const params = new URLSearchParams(location.search);
              ...
              <>
                ...
                <div className="mb-6">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    ...
                    {urlParams.manufacturer && ` ${t('from') || 'from'} ${urlParams.manufacturer}`}
                    {urlParams.model && ` ${urlParams.manufacturer ? '' : t('for') || 'for'} ${urlParams.model}`}
                  </p>
                </div>
