...
import ChatLogHistory from '../../../../shared/components/ChatLogHistory';
...
const InboxPage = () => {
  ...
  const fetchChatHistory = async (senderId, receiverId) => {
    try {
      ...
      // Sort chat history by date in ascending order (oldest first)
  ...
  const openResponseModal = async (message) => {
    setSelectedMessage(message);
    setResponseText('');
    setIsModalOpen(true);
    setChatHistory([]);

    // Fetch chat history using senderId and receiverId from the message
    await fetchChatHistory(message.senderId, message.receiverId);
  };
    ...
    return (
      <div className="space-y-4">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
          ...
          <p className="text-blue-700">There is no inqueri</p>
          <button
