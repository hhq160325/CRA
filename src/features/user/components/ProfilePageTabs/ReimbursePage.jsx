  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-900">Reimburse</h2>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
        <h3 className="text-sm font-semibold text-gray-900 mb-4">Submit a reimbursement</h3>
        <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input name="note" value={form.note} onChange={onChange} placeholder="Optional details" className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
          <div className="md:col-span-2 flex items-center justify-end gap-3">
            <button type="reset" onClick={()=>setForm({ date: '', category: 'Fuel', amount: '', note: '' })} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">Reset</button>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search ref or notes" className="w-56 pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
              ...
              {filtered.map(r => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2">
                    <span className={`px-2 py-0.5 rounded text-xs ${r.status==='approved'?'bg-green-100 text-green-700':r.status==='pending'?'bg-amber-100 text-amber-700':'bg-red-100 text-red-700'}`}>{r.status}</span>
                  </td>
                  <td className="px-3 py-2 text-gray-700 truncate max-w-xs">{r.note}</td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr><td colSpan="6" className="px-3 py-6 text-center text-gray-500">No requests found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
